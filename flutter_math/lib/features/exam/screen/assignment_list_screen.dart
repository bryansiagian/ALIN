...
class _AssignmentListScreenState extends ConsumerState<AssignmentListScreen>
            ...
            return CustomScrollView(
              ...
              slivers: [
                ...
                if (snapshot.connectionState == ConnectionState.waiting)
                  ...
                  SliverFillRemaining(child: _ErrorState(error: snapshot.error.toString(), onRetry: () => setState(_refresh)))
  ...
  Widget _buildSliverAppBar(AsyncSnapshot<List<AssignmentModel>> snapshot) {
    ...
    final canDo = snapshot.data?.where((AssignmentModel a) => (a.attemptLimit - a.examSessionsCount) > 0).length ?? 0;
  ...
  Future<void> _startExam(BuildContext context, WidgetRef ref, AssignmentModel assignment) async {
    try {
          ...
          SnackBar(
            ...
            content: Row(
              children: [
                ...
                Expanded(child: Text("Gagal memulai kuis: $e", style: const TextStyle(color: Colors.white))),
              ],
            ),
          ),
        );
          ...
          child: Column(
            ...
            children: [
              ...
              if (total > 0)
                Row(
                  children: [
                    _StatPill(icon: Icons.list_alt_rounded, label: "$total Kuis", color: Colors.white),
                    const SizedBox(width: 10),
                    _StatPill(
                      icon: Icons.play_circle_outline_rounded,
                      label: "$canDo Bisa Dikerjakan",
                      color: canDo > 0 ? const Color(0xFF6EE7B7) : const Color(0xFFFCA5A5),
                    ),
                  ],
                ),
            ],
...
class _AssignmentCard extends StatefulWidget {
  const _AssignmentCard({required this.item, required this.index, required this.onStart});
...
class _AssignmentCardState extends State<_AssignmentCard> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _pressCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 120), lowerBound: 0.96, upperBound: 1.0, value: 1.0);
      ...
      child: AnimatedBuilder(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(20),
            border: Border.all(
              color: _canAttempt ? _kBlue200 : const Color(0xFFE2E8F0),
              width: 1.5,
            ),
            boxShadow: [
              BoxShadow(
                color: _canAttempt ? _kBlue500.withOpacity(0.08) : Colors.black.withOpacity(0.04),
                blurRadius: 16,
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: (isSafe ? const Color(0xFFEF4444) : _kBlue500).withOpacity(0.30),
                            blurRadius: 10,
                            offset: const Offset(0, 4),
                          ),
                        ],
                      ...
                      child: Icon(
                        isSafe ? Icons.lock_rounded : Icons.assignment_rounded,
                        color: Colors.white,
                        size: 22,
                      ),
                    ),
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              fontWeight: FontWeight.w700,
                              fontSize: 15,
                              color: _canAttempt ? const Color(0xFF0F172A) : const Color(0xFF94A3B8),
                              height: 1.3,
                            ),
                          ),
                          ...
                          Row(
                            children: [
                              _TypeBadge(isSafe: isSafe),
                              const SizedBox(width: 8),
                              if (widget.item.durationMinutes > 0)
                                _InfoChip(
                                  icon: Icons.timer_outlined,
                                  label: "${widget.item.durationMinutes} menit",
                                ),
                            ],
                          ),
                        ],
                ...
                Row(
                  children: [
                    _AttemptIndicator(sisa: _sisa, limit: widget.item.attemptLimit),
                    const Spacer(),
                    _ActionButton(canAttempt: _canAttempt, isLoading: _isLoading),
                  ],
                ),
              ],
            ),
...
class _AttemptIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = pct > 0.5 ? _kBlue500 : pct > 0 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444);
...
class _ActionButton extends StatelessWidget {
  const _ActionButton({required this.canAttempt, required this.isLoading});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(10),
        boxShadow: [BoxShadow(color: _kBlue500.withOpacity(0.35), blurRadius: 8, offset: const Offset(0, 3))],
      ),
      child: isLoading
          ? const SizedBox(
              width: 16,
          ...
          : const Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text("Kerjakan", style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700)),
                SizedBox(width: 6),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const Text("Memuat kuis...", style: TextStyle(color: Color(0xFF64748B), fontSize: 15, fontWeight: FontWeight.w500)),
      ],
    );
  }
}
